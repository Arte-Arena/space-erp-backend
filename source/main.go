...
func main() {
	...
	mux.Handle("GET /v1/leads-by-number/{number}", middlewares.LaravelAuth(http.HandlerFunc(leads.GetOneByNumber)))
	...
	mux.Handle("GET /v1/users/commercial/reports/orders", middlewares.LaravelAuth(http.HandlerFunc(users.GetCommercialOrdersReport)))
	...
	mux.Handle("POST /v1/space-desk/group-to-chat", middlewares.LaravelAuth(http.HandlerFunc(spacedesk.AddGroupToChat)))
