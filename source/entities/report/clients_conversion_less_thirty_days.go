...
func GetClientsConversionLessThirtyDays(from, until string) (int64, error) {
	...
	pipeline := bson.A{
		...
		bson.D{{Key: "$lookup", Value: bson.D{
			...
			{Key: "foreignField", Value: "related_client"},
			{Key: "as", Value: "lead"},
		}}},
		bson.D{{Key: "$unwind", Value: "$lead"}},
		bson.D{{Key: "$addFields", Value: bson.D{
			{Key: "conversion_days", Value: bson.D{
				{Key: "$divide", Value: bson.A{
					bson.D{{Key: "$subtract", Value: bson.A{"$created_at", "$lead.created_at"}}},
					1000 * 60 * 60 * 24,
				}},
			}},
		}}},
		bson.D{{Key: "$match", Value: bson.D{
			{Key: "conversion_days", Value: bson.D{{Key: "$lte", Value: 30}}},
		}}},
