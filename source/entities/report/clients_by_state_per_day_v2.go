...
func GetClientsByStatePerDayV2(from, until string) (map[string]map[string]int64, error) {
	...
	pipeline := bson.A{
		...
		bson.D{{Key: "$group", Value: bson.D{
			{Key: "_id", Value: bson.D{
				...
				{Key: "state", Value: "$contact.state"},
			}},
			{Key: "count", Value: bson.D{{Key: "$sum", Value: 1}}},
		}}},
		bson.D{{Key: "$sort", Value: bson.D{{Key: "_id.year", Value: 1}, {Key: "_id.month", Value: 1}, {Key: "_id.day", Value: 1}, {Key: "_id.state", Value: 1}}}},
	}
	...
	for cursor.Next(ctx) {
		...
		case bson.D:
			for _, elem := range id {
				...
				if elem.Key == "state" && elem.Value != nil {
					state = fmt.Sprintf("%v", elem.Value)
				}
			}
		}
