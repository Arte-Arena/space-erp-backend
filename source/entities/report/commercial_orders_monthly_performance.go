...
func GetCommercialOrdersMonthlyPerformance(seller bson.ObjectID, from, until string) (map[string]float64, error) {
	...
	filter := bson.D{{Key: "related_seller", Value: seller}}
	if from != "" || until != "" {
		dateFilter := bson.D{}
		if from != "" {
