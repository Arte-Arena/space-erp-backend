...
func GetClientsByPersonTypePerDayV2(from, until string) (map[string]map[string]int64, error) {
	...
	pipeline := bson.A{
		...
		bson.D{{Key: "$group", Value: bson.D{
			{Key: "_id", Value: bson.D{
				...
				{Key: "person_type", Value: "$contact.person_type"},
			}},
			{Key: "count", Value: bson.D{{Key: "$sum", Value: 1}}},
		}}},
		bson.D{{Key: "$sort", Value: bson.D{{Key: "_id.year", Value: 1}, {Key: "_id.month", Value: 1}, {Key: "_id.day", Value: 1}, {Key: "_id.person_type", Value: 1}}}},
	}
	...
	for cursor.Next(ctx) {
		...
		case bson.M:
			...
			if pt, ok := id["person_type"]; ok && pt != nil {
				personType = fmt.Sprintf("%v", pt)
			}
		case bson.D:
			for _, elem := range id {
				if elem.Key == "year" {
				...
				if elem.Key == "person_type" && elem.Value != nil {
					personType = fmt.Sprintf("%v", elem.Value)
				}
			}
		}

		if personType == "" {
			personType = "não informado"
		}
		...
		dateKey := fmt.Sprintf("%04d-%02d-%02d", year32, month32, day32)

		if result[dateKey] == nil {
			result[dateKey] = make(map[string]int64)
		}
		result[dateKey][personType] = count64
	}
	return result, nil
}
