...
func GetCommercialBudgetsDailyValue(seller bson.ObjectID, from, until string) (map[string]float64, error) {
	...
	pipeline := bson.A{
		...
		bson.D{{Key: "$addFields", Value: bson.D{
			{Key: "date_string", Value: bson.D{{Key: "$dateToString", Value: bson.D{
		...
		bson.D{{Key: "$group", Value: bson.D{
			{Key: "_id", Value: "$date_string"},
			{Key: "daily_total", Value: bson.D{{Key: "$sum", Value: "$billing.installments.value"}}},
		}}},
	...
	for cursor.Next(ctx) {
		...
		if date, ok := doc["_id"].(string); ok {
			if total, ok := doc["daily_total"].(float64); ok {
				result[date] = total
			} else if total, ok := doc["daily_total"].(int32); ok {
				result[date] = float64(total)
			} else if total, ok := doc["daily_total"].(int64); ok {
				result[date] = float64(total)
			}
		}
	}
