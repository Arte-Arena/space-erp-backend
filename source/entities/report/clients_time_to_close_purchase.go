...
func GetClientsTimeToClosePurchase(from, until string) (float64, error) {
	...
	pipeline := bson.A{
		...
		bson.D{{Key: "$group", Value: bson.D{
			{Key: "_id", Value: "$_id"},
			{Key: "client_created_at", Value: bson.D{{Key: "$first", Value: "$created_at"}}},
		...
		bson.D{{Key: "$addFields", Value: bson.D{
			{Key: "days_to_purchase", Value: bson.D{
				{Key: "$divide", Value: bson.A{
					bson.D{{Key: "$subtract", Value: bson.A{"$first_purchase_at", "$client_created_at"}}},
					1000 * 60 * 60 * 24,
				}},
			}},
		}}},
